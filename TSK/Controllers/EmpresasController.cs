...
namespace TSK.Controllers
{
    ...
    public class EmpresasController : Controller
    {
        ...
        public EmpresasController(USAEU2GIGDEVSQLContext context) {
            _context = context;
        }

        [HttpGet]
        public async Task<IActionResult> Get(DataSourceLoadOptions loadOptions) {
            var empresas = _context.Empresas.Select(i => new {
                i.IdEmp,
                i.Nombre,
                i.Habilitado,
                i.Extracolumn1,
            ...
            // loadOptions.PrimaryKey = new[] { "IdEmp" };
        ...
        public async Task<IActionResult> Post(string values) {
            ...
            var result = _context.Empresas.Add(model);
            await _context.SaveChangesAsync();

            return Json(new { result.Entity.IdEmp });
        }

        [HttpPut]
        public async Task<IActionResult> Put(int key, string values) {
            var model = await _context.Empresas.FirstOrDefaultAsync(item => item.IdEmp == key);
            if(model == null)
        ...
        private void PopulateModel(Empresa model, IDictionary values) {
            string ID_EMP = nameof(Empresa.IdEmp);
            string NOMBRE = nameof(Empresa.Nombre);
            string HABILITADO = nameof(Empresa.Habilitado);
            string EXTRACOLUMN1 = nameof(Empresa.Extracolumn1);
            string EXTRACOLUMN2 = nameof(Empresa.Extracolumn2);
            string EXTRACOLUMN3 = nameof(Empresa.Extracolumn3);
